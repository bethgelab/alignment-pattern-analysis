...
def main(metric, roi, main_outcome, split, plot_path):
    ...
    axes[0].plot(range(len(rois)), brain_mean, color="k",
    ...
    save_this(
        plot_path,
        f"figure_1_v3_{roi}_{metric}_{main_outcome}",
        formats=None,
        transparent=False,
        dpi=300,
    )
    logger.info(f"Saved figure to {plot_path}/figure_1_v3_{roi}_{metric}_{main_outcome}.*")
