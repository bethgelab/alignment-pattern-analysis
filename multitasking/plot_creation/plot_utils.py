...
def set_ticks_0_1(ax=None, start=0, step=0.2, axis="y"):
    """Set y-axis ticks at step intervals (e.g. .2, .4, .6, .8). Does not change the y-axis range."""
    if ax is None:
        ax = plt.gca()
    if axis == "y":
        ylim = ax.get_ylim()
        ticks = np.arange(start, 1 + step / 2, step)
...
def add_noise_ceiling(ax, intersubject_df, roi, split, metric, var_type="std", **kwargs):
    ...
    n_vals = intersubject_df_roi["score"].shape[0]
    if var_type == "std":
        noise_ceiling_std = intersubject_df_roi["score"].std()
    elif var_type == "sem":
        # TODO: check if this is correct
        logger.info(f"Calculating noise ceiling SEM with {n_vals} values")
        noise_ceiling_std = intersubject_df_roi["score"].std() / np.sqrt(n_vals)
