# modified from:
# github.com/scikit-learn/scikit-learn/blob/main/sklearn/linear_model/_ridge.py
...
class _RidgeGCVMod(_RidgeGCV):
    ...
    def __init__(
        ...
        *,
        ...
        self._cv_values_ = None  # Internal variable to store cv_values_

    @property
    def cv_values_(self):
        """Getter for cv_values_."""
        return self._cv_values_

    @cv_values_.setter
    def cv_values_(self, value):
        """Setter for cv_values_."""
        self._cv_values_ = value
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.scoring not in ["pearson_r", "explained_variance"]:
            raise ValueError(
                "modified RidgeCV scoring requires one of "
                "['pearson_r','explained_variance']"
            )
...
class _BaseRidgeCVMod(_BaseRidgeCV):
    def fit(self, X, y, sample_weight=None):
        ...
        if cv is None:
            ...
            if self.store_cv_values:
                self._cv_values_ = estimator.cv_values_
        else:
            if self.store_cv_values:
                raise ValueError(
...
if __name__ == "__main__":
    ...
    LOGGER.info("Best score:", regression.best_score_)
    LOGGER.info("Best alpha:", regression.alpha_)
