...
def prepare_configs(output_path: Path, base_config: benedict):
    ...
    model_names = [
        "v_jepa_vjepa2_vit_giant_384",
        "v_jepa_vjepa2_vit_huge",
        "v_jepa_vjepa2_vit_large",
        "vggt_vggt-1b",
        "vissl_simclr_resnet50_1000epochs",
        "vissl_simclr_resnet101_1000epochs",
    ]

    click.echo(f"Preparing configs for {len(model_names)} models...")

    for model_name in model_names:
        model_config_path = Path(str(CONFIG_ROOT) + "/models/" + model_name + ".yaml")
...
def prepare_slurm_script(output_path: Path, base_config: benedict):
    ...
    source = PROJECT_ROOT / "tasks" / "bold_moments_one_model_mult_layers" / "run.sh"
