...
@click.option(
    ...
    default=CONFIG_ROOT / "benchmark_bold_moments.yaml",
    required=True,
)
@click.option(
    "--output-path",
...
def prepare_configs(output_path: Path, base_config: benedict):
    ...
    # get the model mentioned in the base config
    # model_base_config = "timm_resnet18.a1_in1k"
    # model_base_config = "opt_cwm_opt_cwm"
    model_str = base_config["feature_extraction"]["model"]
    model_base_config = model_str.split("/")[-1].lower()
    # find the config that contains that string  ?  not for now
    all_configs = list(Path(str(CONFIG_ROOT) + "/models/").glob("*.yaml"))
    model_config_paths = [
        p for p in all_configs if model_base_config in str(p).split("/")[-1]
    ]
